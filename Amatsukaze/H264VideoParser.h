...
struct H264SequenceParameterSet {
    ...
    uint8_t vui_parameters_present_flag; // 1å≈íË

    // vui_parameters
    uint8_t aspect_ratio_info_present_flag; // 1å≈íË
    uint8_t aspect_ratio_idc;
    uint16_t sar_width; // 4å≈íË
    uint16_t sar_height; // 3å≈íË
    ...
    uint8_t video_full_range_flag; // 0å≈íË
    uint8_t colour_description_present_flag;
    uint8_t colour_primaries; // 1å≈íË
    uint8_t transfer_characteristics; // 1 or 11
    uint8_t matrix_coefficients; // 1å≈íË

    uint8_t chroma_loc_info_present_flag; // 0å≈íË
    uint32_t chroma_sample_loc_type_top_field;
    uint32_t chroma_sample_loc_type_bottom_field;

    uint8_t timing_info_present_flag; // 1å≈íË
    uint32_t num_units_in_tick; // 1001å≈íË
    uint32_t time_scale; // 60000(29.97) or 120000(59.94)
    uint8_t fixed_frame_rate_flag; // 1å≈íË
    ...
    uint8_t pic_struct_present_flag; // 1å≈íË
...
class H264VideoParser : public AMTObject, public IVideoParser {
    struct NalUnit {
        int offset;
        int length; // rbsp_trailing_bitsÇèúÇ¢ÇΩí∑Ç≥
    };
public:
...
private:
    ...
    // íºëOÇÃ beffering period ÇÃ DTS;
