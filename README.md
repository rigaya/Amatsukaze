...
- MPEG2ソースのCMカット境界のみを再エンコードする部分エンコード
- CMと本編の分離出力
...
1. 「基本設定」タブでtsreplaceへのパスを設定

   tsreplaceは同梱されていないので、[こちら](https://github.com/rigaya/tsreplace/releases)から入手して、パスを設定してください。
...
CMカット位置の前後だけをx262で再エンコードし、それ以外の部分は元のMPEG2の映像をそのままコピーして出力する機能です。本編の大半が元の映像のまま残るので、画質を維持したまま、短時間でCMカットしたTSを作成できます。現状、入力がMPEG2の場合のみ対応しています。

<img src="./data/amatsuakze_tsreplace_partial_enc.png" width="720">

「プロファイル」タブの出力選択の右にある「部分エンコード」をチェックすると有効になります。このチェックボックスは、エンコーダが「x262」で、出力選択が「通常」以外のときに表示されます。

<img src="./data/amatsukaze_mpeg2_partial.png" width="520">

#### 必要な設定

部分エンコードには以下の設定が必要です。条件を満たしていない場合、タスク投入時にエラーになります。

- エンコーダが「x262」
- 出力フォーマットが「TS (replace)」
- 出力選択が「CMをカット」
- チャプター・CM解析が有効
- 入力TSの映像がMPEG2 (4:2:2 Profileを除く)

#### 使用できない設定

元の映像をそのままコピーする都合上、映像に手を加える処理や、出力を分割する処理は使用できません。「部分エンコード」をチェックすると、これらの設定は自動的に無効化されます。
...
素材の構造によっては部分エンコードが行えないことがあります。その場合は理由をログに出力して、自動的に通常のフル再エンコードにフォールバックします。
